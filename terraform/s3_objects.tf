...
resource "aws_s3_object" "docker_compose_web" {
  ...
  content = templatefile("${path.module}/docker-compose.web.yml.tpl", {
    db_private_ip = aws_instance.db.private_ip
    project_name  = var.project_name
  })

  content_type = "text/yaml"
  etag = md5(templatefile("${path.module}/docker-compose.web.yml.tpl", {
    db_private_ip = aws_instance.db.private_ip
    project_name  = var.project_name
  }))
}
...
resource "aws_s3_object" "nginx_web_conf" {
  ...
  content = templatefile("${path.module}/nginx.web.conf.tpl", {
    cloudfront_domain = aws_cloudfront_distribution.wordpress.domain_name
  })

  content_type = "text/plain"
  etag = md5(templatefile("${path.module}/nginx.web.conf.tpl", {
    cloudfront_domain = aws_cloudfront_distribution.wordpress.domain_name
  }))
}
...
resource "aws_s3_object" "env_web" {
  ...
  content = templatefile("${path.module}/env_web.tpl", {
    db_name          = var.db_name
    db_user          = var.db_user
